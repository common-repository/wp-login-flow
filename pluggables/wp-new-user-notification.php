...
if ( ! function_exists( 'wp_new_user_notification' ) ) :
	...
	function wp_new_user_notification( $user_id, $deprecated = null, $notify = '' ) {
		...
		if ( 'user' !== $notify ) {
			...
			$message = sprintf( __( 'New user registration on your site %s:', 'wp-login-flow' ), $blogname ) . "\r\n\r\n";
			/* translators: %s: user login */
			$message .= sprintf( __( 'Username: %s', 'wp-login-flow' ), $user->user_login ) . "\r\n\r\n";
			/* translators: %s: user email address */
			$message .= sprintf( __( 'Email: %s', 'wp-login-flow' ), $user->user_email ) . "\r\n";

			$wp_new_user_notification_email_admin = array(
				'to'      => get_option( 'admin_email' ),
				/* translators: Password change notification email subject. %s: Site title */
				'subject' => __( '[%s] New User Registration', 'wp-login-flow' ),
				'message' => $message,
				'headers' => '',
			);
		...
		if( ! $user_set_pw ){
			...
			$default_subject = sprintf( __( '[%s] Account Information', 'wp-login-flow' ), get_option( 'blogname' ) );
			$default_message = sprintf( __( "Welcome to %s! Your account information is as follows:", 'wp-login-flow' ), get_option( 'blogname' ) ) . "\r\n\r\n";
			$default_message .= sprintf( __( "Username: %s", 'wp-login-flow' ), '%wp_user_name%' ) . "\r\n\r\n";
			$default_message .= __( "You can login to your account at the URL below:", 'wp-login-flow' ) . "\r\n\r\n" . '<a href="%wp_login_url%">%wp_login_url%</a>';

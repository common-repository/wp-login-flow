 ...
 * Version:     3.1.1
 * Last Updated: Wed Oct 07 2020 13:18:54
...
Class WP_Login_Flow {
	...
	public static function admin_notices() {
		...
		if ( WP_LOGIN_FLOW_VERSION != get_option( 'WP_LOGIN_FLOW_VERSION' ) ) {
			...
			$html .= __( 'WP Login Flow is now activated, you can find <strong>Login Flow</strong> under the <strong>Users</strong> menu.', 'wp-login-flow' );
			$html .= '</p></div>';
		...
		if( is_multisite() && ! get_option( 'WP_LOGIN_MS_NOTICE' ) ){
			...
			$html .= '<p>' . __( 'WP Login Flow is recommended for single site installations -- not multisite installations.  All the features "should" work, but PLEASE thoroughly test everything manually, and report any issues you find on GitHub.', 'wp-login-flow' );
			$html .= '</p></div>';
		...
		if ( $is_nginx && ! get_option( 'WP_LOGIN_NGINX_NOTICE' ) ) {
			$html = '<div class="error">';
			$html .= '<p style="float:right;"><a href="' . esc_url( add_query_arg( "dismiss-wplf-nginx-notice", "1" ) ) . '">' . __( 'Hide notice', 'wp-login-flow' ) . '</a></p>';
			$html .= '<p>' . __( 'WP Login Flow detected you are using Nginx for your web server.  Because of this we can NOT automatically set the rewrites in your config, you MUST manually configure them! YOU MUST ALSO ENABLE REWRITES UNDER THE PERMALINKS TAB ON WP LOGIN FLOW CONFIG PAGE IF YOU HAVE AUTO DISABLE ENABLED!', 'wp-login-flow' );
			$html .= '</p></div>';
	...
	public function settings_plugin_link( $links ){
		...
		$links[] = '<a href="' . admin_url( '/users.php?page=wp-login-flow' ) . '">' . __( 'Settings', 'wp-login-flow' ) . '</a>';
		return $links;
	}
	...
	public function add_plugin_row_meta( $plugin_meta, $plugin_file, $plugin_data, $status ) {
		...
		if ( self::PLUGIN_SLUG . '/' . self::PLUGIN_SLUG . '.php' == $plugin_file ) {
			...
			$plugin_meta[] = '<a href="' . admin_url( '/users.php?page=wp-login-flow' ) . '">' . __( 'Settings', 'wp-login-flow' ) . '</a>';
			$plugin_meta[ ] = '<a target="_blank" href="http://github.com/tripflex/' . self::PLUGIN_SLUG . '">' . __( 'Open Source', 'wp-login-flow' ) . '</a>';

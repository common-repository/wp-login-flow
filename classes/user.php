...
class WP_Login_Flow_User extends WP_Login_Flow {
	...
	function change_user_strings( $text ) {
		...
		if ( $text === 'Send Password?' ) return __( 'Require Activation?', 'wp-login-flow' );
		if ( $text === 'Send this password to the new user by email.' ) return __( 'Send an email to new user with link to activate and set password.', 'wp-login-flow' );

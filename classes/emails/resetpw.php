...
class WP_Login_Flow_Emails_ResetPW extends WP_Login_Flow_Emails {
	...
	function message( $message, $key, $user_login, $user_data ){
		...
		$title = get_option( 'wplf_lostpassword_subject', sprintf( __( '[%s] Password Reset', 'wp-login-flow' ), $blogname ) );
		...
		if ( $message && ! wp_mail( $user_data->user_email, wp_specialchars_decode( $title ), $message, 'Content-type: text/html' ) )
			wp_die( __( 'The e-mail could not be sent.', 'wp-login-flow' ) . "<br />\n" . __( 'Possible reason: your host may have disabled the mail() function.', 'wp-login-flow' ) );

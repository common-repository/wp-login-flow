...
class WP_Login_Flow_Register extends WP_Login_Flow_Login {
	...
	function registration_complete( $errors, $redirect_to ) {
		...
		if( is_wp_error( $errors ) && $errors->error_data && array_key_exists( 'registered', $errors->error_data ) && $errors->error_data['registered'] === 'message' ){
			...
			if( get_option( "wplf_register_set_pw", false ) && ! get_option( "wplf_auto_login", false ) ){
				$errors = new WP_Error();
				$errors->add( 'registered', __( 'Registration complete. You can now login to your account below.', 'wp-login-flow' ), 'message' );
			}
	...
	function check_for_string_changes( $text ) {
		...
		if( $text === 'Username or Email Address' && get_option( 'wplf_registration_email_as_un', false ) ){
			return __( 'Email Address', 'wp-login-flow' );
		}
	...
	public function output_pw_fields(){
		...
		if ( get_option( "wplf_register_set_pw", false ) ) {
			...
			<div class="user-pass1-wrap">
				<p>
					<label for="pass1"><?php _e( 'Password', 'wp-login-flow' ); ?></label>
				</p>
				...
				<div class="wp-pwd">
					...
					<div id="pass-strength-result" class="hide-if-no-js" aria-live="polite"><?php _e( 'Strength indicator', 'wp-login-flow' ); ?></div>
				</div>
				<div class="pw-weak">
					<label>
						<input type="checkbox" name="pw_weak" class="pw-checkbox"/>
						<?php _e( 'Confirm use of weak password', 'wp-login-flow' ); ?>
					</label>
				</div>
			</div>

			<p class="user-pass2-wrap">
				<label for="pass2"><?php _e( 'Confirm new password', 'wp-login-flow' ); ?></label><br/>
	...
	public function registration_errors( $errors, $sanitized_user_login, $user_email ) {
		...
		if ( get_option( "wplf_register_set_pw", false ) ) {
			if ( empty( $_POST['pass1'] ) || ( ! empty( $_POST['pass1'] ) && trim( $_POST['pass1'] ) == '' ) ) {
				$errors->add( 'password1_error', __( '<strong>ERROR</strong>: Password field is required.', 'wp-login-flow' ) );
			}
		}
			...
			foreach( $custom_fields as $custom_field ){
				...
				if( array_key_exists( 'required', $custom_field ) && ! empty( $custom_field['required'] ) ){
					...
					if( empty( $custom_value ) ){
						$errors->add( "{$mk}_error", sprintf( __( '<strong>ERROR</strong>: %s field is required.', 'wp-login-flow' ), $custom_field['label'] ) );
						break;
					}

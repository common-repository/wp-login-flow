...
//		add_filter( 'validate_password_reset', array( $this, 'activation_password_set' ), 9999, 1 );
	...
	function check_for_string_changes( $text ){
		...
		if( $text === 'Reset Password' && ( $this->get_step() === 'activate' || $this->get_step() === 'setpw' ) ){
			return __( 'Set Password', 'wp-login-flow' );
		}
	...
	function activation_pending_page() {
		...
		$thankyou_notice = sprintf( __( '<p>Thank you for registering.  Please check your email for your activation link.</p><p>If you do not receive the email please request a <a href="%s">password reset</a> to have the email sent again.</p>', 'wp-login-flow' ), '%wp_lost_pw_url%' );
		$template        = new WP_Login_Flow_Template();
		$thankyou_notice = $template->generate( 'wplf_notice_activation_required', $thankyou_notice );
		login_header( __( 'Pending Activation', 'wp-login-flow' ), '<div class="message reset-pass">' . $thankyou_notice . '</div>' );
		login_footer();
		exit;

...
class WP_Login_Flow_User_Auth extends WP_Login_Flow_User {
	...
	function check( $user, $username, $password ) {
		...
		if ( ! $this->activation()->check( $user_id ) ) {
			$user           = new WP_Error();
			$pending_notice = sprintf( __( '<strong>ERROR</strong>: Your account is still pending activation, please check your email, or you can request a <a href="%s">password reset</a> for a new activation code.', 'wp-login-flow' ), '%wp_lost_pw_url%' );

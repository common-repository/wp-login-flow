...
class WP_Login_Flow_Settings_Fields {
	...
	function check_permalinks( $o ){
		...
		echo "<div class=\"wplf-rewrite-conflict\">" . __( 'This rewrite conflicts with the ', 'wp-login-flow' ) . "<a href=\"" . get_edit_post_link( $check_page->ID ) . "\" target=\"_blank\">{$check_page->post_title} {$check_page->post_type}</a>" . __( ' permalink!', 'wp-login-flow' ) . "<br /><small>" . __( 'Your rewrites for WP Login Flow should take precendence over any permalinks but this also means that page will not load correctly now!', 'wp-login-flow' ) . "</small><br/><small>" . __( "You should probably use a different permalink or rewrite.", 'wp-login-flow' ) . "</small></div>";
...
<?php
	...
	function repeatable_field_template( $a, $value = false, $index = false ){
		...
		if ( isset( $o['rfields'] ) && ! empty( $o['rfields'] ) ) {
			...
			if( ! $existing_output ){
				echo "<div class=\"repeatable-add-group-row\"><button class=\"button repeatable-add-group-row-button\" type=\"button\" data-group=\"" . $o['name'] . "\" data-rowtemplate=\"group-" . $o['name'] . "-tmpl\">" . __( 'Add Another', 'wp-login-flow' ) . "</button></div>\r\n";
...
//				$label     = '{{label_' . $field . '}}';
				...
				if ( $c === count( $o['rfields'] ) ) {
					...
					echo "  <div data-group=\"{$o['name']}\" class=\"button button-primary right repeatable-fields-remove-group-row\">" . __( 'Remove', 'wp-login-flow' ) . '</div>';
					echo '</td></tr>';
				}
			}
			echo "		</tbody>\r\n";
